...
/*   Updated: 2021/04/27 16:26:39 by azziz            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char			get_flags(t_elf_symbol sym, t_elf_section *sections) {
	...
	/* prints(" "); */
	/* ft_putnbr((sections[sym.shndx].flag & SHF_MASKOS) | SHF_ALLOC); */
	/* prints(" "); */
	...
	if (sections[sym.shndx].name == NULL) {
		...
		if (sections[sym.shndx].flag == (SHF_ALLOC | SHF_EXECINSTR)) {
			...
			if (sym.bind == STB_WEAK) {
				...
				if (sym.type == STT_OBJECT)
					return (sym.shndx == SHN_UNDEF ? 'g' : 'G');
			}
		...
		if (sections[sym.shndx].flag == (SHF_ALLOC | SHF_WRITE)) {
			if (sym.type == STT_OBJECT)
				return (sym.bind == STB_LOCAL ? 'V' : sym.bind == STB_GLOBAL ? 'B' : 'S');
