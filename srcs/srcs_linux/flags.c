...
/*   Updated: 2021/04/27 14:48:05 by azziz            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char			get_flags(t_elf_symbol sym, t_elf_section *sections) {
	...
	prints(" ");
	ft_putnbr(sections[sym.shndx].flag & SHF_MASKPROC);
	prints(" ");
	if (sections[sym.shndx].name == NULL) {
		...
		else if (sections[sym.shndx].flag == sections[sym.shndx].flag | SHF_ALLOC & SHF_MASKOS)
			return (sym.bind == STB_LOCAL ? 'r' : 'R');
		else if (sections[sym.shndx].flag == sections[sym.shndx].flag | (SHF_ALLOC | SHF_WRITE))
