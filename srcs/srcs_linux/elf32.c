...
/*   Updated: 2021/05/06 12:28:46 by azziz            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int					print_symelf(Elf32_Sym *sym, Elf32_Shdr *sh, Elf32_Ehdr *eh, int idx, t_elf_section *sections) {
	...
	for (i = 0; i < symcnt; i++) {
		if (sym[i].st_name != 0 && ELF32_ST_TYPE(REV32(sym[i].st_info, rev)) != STT_FILE
				&& ELF32_ST_TYPE(REV32(sym[i].st_info, rev)) != STT_SECTION) {
	...
	for (i = 0; i < j; i++) {
		print_symbols(symbols[i], sections, sh, REV32(eh->e_machine, rev), REV32(eh->e_shnum, rev));
	}
	free(symbols);
	return (EXIT_SUCCESS);
}
